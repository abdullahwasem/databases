...
CREATE VIEW one AS 
SELECT OrderNo, (Quantity*items.Price) as totalprice
FROM OrderDetails inner join items on items.ItemNo = OrderDetails.ItemNo
select *from one


CREATE VIEW two AS
SELECT items.Itemno
FROM items  inner join OrderDetails  on items.itemno = OrderDetails.ItemNo
where OrderDetails.Quantity>20;

select *from two;


create view three as
select Customers.Name from Customers inner join orders on Customers.CustomerNo = Orders.CustomerNo
where orders.OrderNo = any(select orders.OrderNo from one inner join orders on one.OrderNo = Orders.OrderNo where totalprice > 20000)

select * from three;


create view four as
(select customers.Name from customers) except (select Customers.Name from Customers where phone is null);


create view foura as
(select customers.Name from customers) except (select Customers.Name from Customers where phone is null) with check option ;
